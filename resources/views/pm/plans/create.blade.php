...
<div class="space-y-4">
  ...
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="text-sm text-gray-600">Buat template preventive maintenance per kategori aset.</p>
    </div>
  ...
  <form method="post" action="{{ route('pm.plans.store') }}" class="bg-white rounded-2xl shadow p-3 md:p-4">
    @csrf
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        ...
        <input type="text" name="nama_plan" value="{{ old('nama_plan') }}"
               class="w-full rounded-xl border-gray-300 focus:ring-sky-500 focus:border-sky-500" placeholder="Contoh: PM Mesin Potong Bulanan" required>
      </div>
      ...
      <div>
        ...
        <select name="asset_category_id" class="w-full rounded-xl border-gray-300 focus:ring-sky-500 focus:border-sky-500" required>
          ...
          @foreach($categories as $c)
            <option value="{{ $c->id }}" @selected(old('asset_category_id')==$c->id)>{{ $c->nama }}</option>
          @endforeach
        </select>
      </div>
      ...
      <div>
        ...
        <select name="interval_type" id="interval_type"
          ...
          @foreach($intervalOptions as $k=>$v)
            <option value="{{ $k }}" @selected(old('interval_type')===$k)>{{ $v }}</option>
          @endforeach
        </select>
      ...
      <div>
        ...
        <input type="number" min="1" step="1" name="interval_value" value="{{ old('interval_value', 1) }}"
               class="w-full rounded-xl border-gray-300 focus:ring-sky-500 focus:border-sky-500" placeholder="Mis. 1, 2, 3…" required>
      </div>
      ...
      <div>
        ...
        <select name="default_assignee_id" class="w-full rounded-xl border-gray-300 focus:ring-sky-500 focus:border-sky-500">
          ...
          @foreach($pjs as $u)
            <option value="{{ $u->id }}" @selected(old('default_assignee_id')==$u->id)>{{ $u->name }} @if($u->divisi) — {{ $u->divisi }} @endif</option>
          @endforeach
        </select>
      </div>
      ...
      <div class="flex items-end">
        <label class="inline-flex items-center gap-2">
          <input type="checkbox" name="aktif" value="1" class="rounded border-gray-300 text-sky-600 focus:ring-sky-500"
                 @checked(old('aktif', 1))>
          <span class="text-sm">Aktifkan plan setelah dibuat</span>
        </label>
      </div>
    </div>
    ...
    <div class="mt-4">
      ...
      <div id="checklistWrap" class="space-y-2">
        @php
          ...
          if (empty($oldChecklist)) { $oldChecklist = ['']; }
        @endphp
        @foreach($oldChecklist as $i => $val)
          <div class="flex gap-2">
            <input type="text" name="checklist[]" value="{{ $val }}" required
                   class="flex-1 rounded-xl border-gray-300 focus:ring-sky-500 focus:border-sky-500" placeholder="Deskripsi tugas (mis. Bersihkan filter, cek tegangan, ganti oli)">
    ...
    <div class="mt-5 flex flex-col md:flex-row gap-2 md:justify-end">
      ...
      <button class="rounded-xl bg-gray-900 text-white px-4 py-2 hover:bg-black">Simpan Plan</button>
    </div>
  </form>
</div>
    ...
    wrap?.addEventListener('click', function(e) {
      if (e.target && e.target.classList.contains('btnRemoveRow')) {
        ...
        if (rows.length > 1) {
          ...
          // jika tinggal 1, kosongkan saja biar tetap valid
